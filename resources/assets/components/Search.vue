<template>
  <div id="search">
    ...
    <div class="container-fluid" v-if="!isLoadingPgae">
      <!-- title page -->
      <h3 class="my-2">{{ $t('SEARCH.title') }}</h3>
        ...
        <div class="col-12">
          ...
          <div class="d-flex justify-content-between align-items-center w-100">
            <div class="d-flex app">
              <div class="guide" v-if="$t('SEARCH.search.GUIDE')">
                <p>{{ $t('SEARCH.search.GUIDE') }}</p>
              </div>
              <h5 class="m-0">{{ $t('SEARCH.search.title') }}</h5>
            </div>
          ...
          <table class="mt-4">
            <thead>
              <tr>
                <th>{{ $t('SEARCH.search.date') }}</th>
                <th>{{ $t('SEARCH.search.queue') }}</th>
                <th>{{ $t('SEARCH.search.agent') }}</th>
                <th>{{ $t('SEARCH.search.event') }}</th>
                <th>{{ $t('SEARCH.search.wait') }}</th>
                <th>{{ $t('SEARCH.search.time') }}</th>
                <th>{{ $t('SEARCH.search.number') }}</th>
                <th>{{ $t('SEARCH.search.callid') }}</th>
              </tr>
            </thead>
            <tbody>
              ...
              <tr style="height: 20px">
                <td>
                  ...
                  <date-picker v-model="timeFilter" type="date" custom-input=".date " range clearable>
                    <!-- slot for "now-btn" -->
...
export default {
  ...
  data() {
    return {
      ...
      optionTimeFilter: [
        { code: 'today', lable: this.$t(`HOME.today`) },
        { code: 'yesterday', lable: this.$t(`HOME.yesterday`) },
        { code: 'lastWeek', lable: this.$t(`HOME.lastWeek`) },
        { code: 'currentWeek', lable: this.$t(`HOME.currentWeek`) },
        { code: 'currentMonth', lable: this.$t(`HOME.currentMonth`) },
        { code: 'last3Month', lable: this.$t(`HOME.last3Month`) },
      ],
