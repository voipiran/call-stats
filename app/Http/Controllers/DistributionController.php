...
class DistributionController extends Controller
{
	...
	public function getDetail($request)
	{
		try {
			...
			$Unanswered = $this->CalculationDetail($request, ['RINGNOANSWER']);
	...
	public function getWaitByDate($request)
	{
		try {
			...
			$Unanswered = $this->CalculationWaitByDate($request, ['RINGNOANSWER'], 'Unanswered');
	...
	public function getWaitByTime($request)
	{
		try {
			...
			$Unanswered = $this->CalculationWaitByTime($request, ['RINGNOANSWER'], 'Unanswered');
	...
	public function getAnsweredInWeek($request)
	{
		try {
			...
			$Unanswered = $this->CalculationAnsweredInWeek($request, ['RINGNOANSWER'], 'Unanswered');
	...
	public function getAnsweredInMonth($request)
	{
		try {
			...
			$Unanswered = $this->CalculationAnsweredInMonth($request, ['RINGNOANSWER'], 'Unanswered');
	...
	public function CalculationAnsweredInMonth($request, $event, $name, $where = false)
	{
		try {
			$calc = DB::table('queue_stats')
				->select(DB::raw('Month(time) as month'), DB::raw("COUNT(agent) as count$name"), DB::raw("ROUND(AVG(data1),2) as data1$name"), DB::raw("ROUND(AVG(data2),2) as data2$name"), DB::raw("SUM(data3) as data3$name"))
				->groupBy('month')
				->whereIn('agent', $request->agents)
				->whereIn('queuename', $request->queues)
				->whereIn('event', $event);
